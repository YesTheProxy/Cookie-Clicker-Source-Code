...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.bakeryNameRefresh=function()
		{
			...
			if (name.indexOf('Cookie',name.length-('Cookie').length)>0 && !Game.sesame) Game.OpenSesame();
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					'<q>Come give Cookie a kiss.</q><sig>Cookie</sig>',
					'<q>6165503944 call me...</q><sig>Cookie</sig>',
					...
					'<q>Thanks for using Ram Networks</q><sig>grandpa Cookie</sig>',
					'<q>Kill The Grandmas</q><sig>Cookie</sig>',
